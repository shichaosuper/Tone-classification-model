...
def run_net(learning_rate, training_iters, batch_size, display_step, n_input, n_classes, dropout, n_channel):
    ...
    datasets = data_loader.load_data(training_iters, batch_size)

    train_set_x, train_set_y = theano.shared(numpy.array(datasets[0][0])), theano.shared(numpy.array(datasets[0][1]))
    ...
    cost, y_pred = softmax_cross_entropy_with_logits(out.output, y)

    # create a function to compute the mistakes that are made by the model
    test_model = theano.function([], T.mean(T.eq(T.argmax(theano.tensor.nnet.nnet.softmax(out.output), axis = 1), y)),
             givens={
                x: test_set_x,
                y: test_set_y})
    ...
    train_model = theano.function([index], [cost, T.mean(T.eq(y_pred, y))], updates=updates,
          givens={
...
run_net(learning_rate = 0.002, training_iters = 100000, batch_size = 66, display_step = 100, n_input = 40, n_classes = 4, dropout = 0.9, n_channel = 1)